...
(defmethod parse* :arrow [[tag {:keys [a b]}]]
  (Arrow. (parse* a) (parse* b)))

(defmethod parse* :tuple [[tag expr]]
  (Tuple. (mapv parse* expr)))
...
(defmethod parse* :pred [[tag {:keys [expr] {:keys [op args]} :pred}]]
  (Predicated. [(Predicate. (second op) (mapv parse* args))] (parse* expr)))
